...
const ZainDXContent = ({ apiData }) => {
  const t = useTranslations("zainDX");
  const tCommon = useTranslations("common");
  const tServices = useTranslations("services");

  const pageContent = apiData?.pages?.[0]?.content;
  const faqs = apiData?.faqs ?? [];
  const useApiContent = pageContent != null && pageContent !== "";
  const useApiFaqs = Array.isArray(faqs) && faqs.length > 0;
          ...
          <div className="business-unit-details-items">
            <div className="row">
              <div className="col-xl-8 col-lg-7 order-lg-last pl-50 pl-md-15 pl-xs-15">
                {useApiContent ? (
                  <div
                    className="zain-dx-page-content mb-50"
                    ...
                    {useApiFaqs
                      ? faqs.map((faq, index) => (
                          <ServiceAccordionItem
                            key={faq.id}
                            serviceKey={`faq-${faq.id}`}
                            title={faq.question}
                            answer={
                              faq.answer ? decodeHtmlEntities(faq.answer) : ""
                            }
                            index={index}
                            isFirst={index === 0}
                          />
              ...
              <div className="col-xl-4 col-lg-5 mt-md-120 mt-xs-50 services-sidebar">
                ...
                >
                  <div className="content">
                    ...
                    <p>{tServices("contactSupportText")}</p>
                    <h3>
                      <a
                        href="tel:+96872720046"
                        style={{ color: "white" }}
                        dir="ltr"
                      >
                        +968 7272 0046
                ...
                <div className="single-widget services-list-widget mt-40">
                  ...
                  <div className="content">
                    <ul>
                      <li>
                        <Link href="/zain-infra">{tCommon("zainInfra")}</Link>
                      </li>
                      <li className="current-item">
                        <Link href="/zain-dx">{tCommon("zainDX")}</Link>
                      </li>
                    </ul>
                  </div>
                </div>
